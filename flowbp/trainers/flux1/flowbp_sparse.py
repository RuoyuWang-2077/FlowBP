...
def _sample_active_indices(
    ...
    """Sample ``num_active`` distinct indices uniformly from ``[start_idx, end_idx)``."""
...
def _compose_x0_from_velocities(
    ...
    For active indices the freshly-forwarded differentiable velocities are
    used; for the rest the cached detached velocities are used. Returns
    fp32 — the connector stays in fp32 to match LeapAlign's precision path;
    autocast at the transformer/VAE boundary handles down-casting.
    """
...
class FlowBPSparseFluxTrainer(LeapAlignFluxTrainer):
    ...
    while backward flows only through the K active velocities.
    """
