...
"""FlowBP-Bridge trainer for FLUX.2.

Mirrors the FlowBP-Bridge FLUX.1 trainer: roll out once, pick a
random ``j`` to split the cached trajectory into ``0 -> j`` and ``j -> x_0``,
sample active velocities from both sides whenever possible, and reconstruct both
segments by exact Euler composition over cached sigma / velocity history. The
straight-through ``x_j`` keeps the forward path unbiased while restoring one
nested gradient layer through the second segment.
...
from flowbp.trainers.flux1.flowbp_sparse import _sample_active_indices
from flowbp.trainers.flux2.flowbp_sparse import get_flowbp_sparse_reward_loss
...
def _sample_post_indices_force_j(
    ...
    slots are drawn uniformly from ``[j_idx + 1, j_idx + post_len)``.
    """
...
def sample_flowbp_bridge_trajectory(
    ...
    d_0 = torch.abs(x0_true.double() - x0_hat.double()).mean(
        dim=tuple(range(1, x0_true.ndim)), keepdim=False,
    )

    sample_weights = torch.ones(
        x0_conn.shape[0], device=device, dtype=torch.float32,
    )
    pre_idx_tensor = torch.tensor(pre_active_indices, device=device, dtype=torch.float32)
    post_idx_tensor = torch.tensor(
        post_active_indices, device=device, dtype=torch.float32,
    )
    ...
    connector_payload = {
        ...
        "connector_scheme": "flowbp_bridge_euler_flux2",
    }
    _maybe_save_connector_dump(args, "flowbp_bridge_flux2", connector_payload)
    args._last_connector_payload = connector_payload

    return x0_conn, sample_weights, traj_metrics


def train_flowbp_bridge_one_step_flux2(
    args,
    inner_step,
    device,
    transformer,
...
class FlowBPBridgeFlux2Trainer(LeapAlignFlux2Trainer):
    """FLUX.2 trainer wrapper for FlowBP-Bridge."""
    ...
    def train_one_step(
        ...
        return train_flowbp_bridge_one_step_flux2(
            self.args,
            inner_step,
            device,
            transformer,
